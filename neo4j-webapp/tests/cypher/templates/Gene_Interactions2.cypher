...
WHERE ( ANY (key in keys(gene) WHERE gene[key]='notch') AND (gene)-[]-(Organism { shortName: 'D. melanogaster' } )) AND gene_interactions_details_experiment_interactiondetectionmethods.name = 'two hybrid' AND gene_interactions_details.type = 'physical'
